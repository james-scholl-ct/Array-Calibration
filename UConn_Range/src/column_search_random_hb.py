...
MAIN_LOBE_HALF_WIDTH = 5 #Number of points in scan for the main lobe half width
...
final_voltages = np.zeros((24,8))
v_arr = np.zeros((24,8))
for run in range(3):
    rand_rows = np.random.permutation(24)
    final_losses = []
    final_patterns = []
...
update_hb_array_file(zero_volts)
rpi.update_dacs()
time.sleep(LC_DELAY_TIME)

...
(
    ...
    //              +----------------+-----------------------+
    localparam integer MEM_N_BLOCKS = 4;
    ...
    // Parameters: Peak Detect
    localparam integer PK_DET_DATA_N_BITS = 16;
    localparam integer PK_DET_CNT_N_BITS = 12;
    localparam integer PK_DET_NUM_WINDOWS_N_BITS = 8;
    localparam integer PK_DET_NOISE_EST_N_BITS = 16;
    localparam integer PK_DET_BUNDLE_N_BITS = 88;
    ...
    // ------------------------------------------------------------------------
    localparam integer PK_DET_WORD_OFFSET   = 8;
    ...
    wire [2 : 0]    debug_ch_sel;
    wire [3 : 0]    cmd_fifo_sig_sel;
    ...
    assign debug_ch_sel = active_mem_u[DEBUG_WORD][7 : 5];
    assign cmd_fifo_sig_sel = active_mem_u[DEBUG_WORD][11 : 8];

    // - peak detector
    wire        [PK_DET_CNT_N_BITS - 1 : 0]         pk_det_guard_samples;
    wire                                            pk_det_mode;
    wire        [PK_DET_CNT_N_BITS - 1 : 0]         pk_det_sample_idx_off;
    wire        [PK_DET_CNT_N_BITS - 1 : 0]         pk_det_sample_idx_on;
    wire        [PK_DET_NUM_WINDOWS_N_BITS - 1 : 0] pk_det_num_windows;
    wire signed [PK_DET_DATA_N_BITS - 1 : 0]        pk_det_prog_fa_thresh;
    wire                                            pk_det_prog_fa_thresh_en;

    assign pk_det_guard_samples     = active_mem_u[PK_DET_WORD_OFFSET + 0][PK_DET_CNT_N_BITS - 1 : 0];
    assign pk_det_mode              = active_mem_u[PK_DET_WORD_OFFSET + 1][0];
    assign pk_det_sample_idx_off    = active_mem_u[PK_DET_WORD_OFFSET + 2][PK_DET_CNT_N_BITS - 1 : 0];
    assign pk_det_sample_idx_on     = active_mem_u[PK_DET_WORD_OFFSET + 2][PK_DET_CNT_N_BITS +: PK_DET_CNT_N_BITS];
    assign pk_det_num_windows       = active_mem_u[PK_DET_WORD_OFFSET + 3][PK_DET_NUM_WINDOWS_N_BITS - 1 : 0];
    assign pk_det_prog_fa_thresh    = active_mem_u[PK_DET_WORD_OFFSET + 4][PK_DET_DATA_N_BITS - 1 : 0];
    assign pk_det_prog_fa_thresh_en = active_mem_u[PK_DET_WORD_OFFSET + 5][0];
    ...
    reg                                  debug_y_dv_pp;
    wire signed [15 : 0]                 interp_y [7 : 0];
    wire                                 interp_y_dv [7 : 0];
    ...
    begin
        case (rsp_fifo_sig_sel)
            ...
            4'd1:
            begin
                debug_y = interp_y[debug_ch_sel];
                debug_y_dv = interp_y_dv[debug_ch_sel];
            end
    ...
    // Interpolation
    localparam INTERP_X_N_BITS = 18;
    localparam INTERP_H_N_BITS = 16;
    localparam INTERP_H0_N_WORDS = 9;
    localparam INTERP_H1_N_WORDS = 5;
    localparam INTERP_H2_N_WORDS = 5;

    wire signed [INTERP_H_N_BITS - 1 : 0] interp_h0_u [INTERP_H0_N_WORDS - 1 : 0];
    wire signed [INTERP_H_N_BITS - 1 : 0] interp_h1_u [INTERP_H1_N_WORDS - 1 : 0];
    wire signed [INTERP_H_N_BITS - 1 : 0] interp_h2_u [INTERP_H2_N_WORDS - 1 : 0];
    wire        [INTERP_H_N_BITS * INTERP_H0_N_WORDS - 1 : 0]   interp_h0;
    wire        [INTERP_H_N_BITS * INTERP_H1_N_WORDS - 1 : 0]   interp_h1;
    wire        [INTERP_H_N_BITS * INTERP_H2_N_WORDS - 1 : 0]   interp_h2;

    // - TODO: Patmos drive coefficients from PBE memory map.
    // - Coefficients are formatted as signed with no integer bits: s0.(N-1).
    assign interp_h0_u[0]=     -21;
    assign interp_h0_u[1]=     116;
    assign interp_h0_u[2]=    -341;
    assign interp_h0_u[3]=     786;
    assign interp_h0_u[4]=   -1589;
    assign interp_h0_u[5]=    3054;
    assign interp_h0_u[6]=   -6226;
    assign interp_h0_u[7]=   20608;
    assign interp_h0_u[8]=   32767;

    assign interp_h1_u[0]=     -44;
    assign interp_h1_u[1]=     834;
    assign interp_h1_u[2]=   -4112;
    assign interp_h1_u[3]=   19722;
    assign interp_h1_u[4]=   32767;

    assign interp_h2_u[0]=     -44;
    assign interp_h2_u[1]=     834;
    assign interp_h2_u[2]=   -4112;
    assign interp_h2_u[3]=   19722;
    assign interp_h2_u[4]=   32767;

    // - pack three sets of coefficients (little-endian) for delivery to each
    // - of the three interpolation stages.
    generate
        for (i_gv = 0; i_gv < INTERP_H0_N_WORDS; i_gv = i_gv + 1)
        begin : pack_interp_h0
            assign interp_h0[i_gv * INTERP_H_N_BITS +: INTERP_H_N_BITS] = interp_h0_u[i_gv];
        end

        for (i_gv = 0; i_gv < INTERP_H1_N_WORDS; i_gv = i_gv + 1)
        begin : pack_interp_h1
            assign interp_h1[i_gv * INTERP_H_N_BITS +: INTERP_H_N_BITS] = interp_h1_u[i_gv];
        end

        for (i_gv = 0; i_gv < INTERP_H2_N_WORDS; i_gv = i_gv + 1)
        begin : pack_interp_h2
            assign interp_h2[i_gv * INTERP_H_N_BITS +: INTERP_H_N_BITS] = interp_h2_u[i_gv];
        end
    endgenerate

    // - Assuming 16-bit datapath for now. Actual sizing is pending system
    // - analysis. There's no reason the sizes should all be equal.
    bcuda_interp #(
        .X_N_BITS               (INTERP_X_N_BITS),
        .FIR0_H_N_BITS          (INTERP_H_N_BITS),
        .FIR0_Y_N_BITS          (16),
        .FIR1_H_N_BITS          (INTERP_H_N_BITS),
        .FIR1_Y_N_BITS          (16),
        .FIR2_H_N_BITS          (INTERP_H_N_BITS),
        .FIR2_Y_N_BITS          (16))
    i_interp (
        .CLK                    (s00_axi_aclk),
        .RESET                  (~s00_axi_aresetn),
        ...
        .H1                     (interp_h1),
        .H2                     (interp_h2),
        .X                      (cmd_fifo_rdata[INTERP_X_N_BITS - 1 : 0]),
        .X_DV                   (cmd_fifo_rvalid),
    ...
    // Peak detector
    wire pk_det_block_valid;

    bcuda_pk_det #(
        .DIN_N_BITS             (PK_DET_DATA_N_BITS),
        .THRESH_N_BITS          (PK_DET_DATA_N_BITS),
        .PEAK_VAL_N_BITS        (PK_DET_DATA_N_BITS),
        .GUARD_SAMPLES_N_BITS   (PK_DET_CNT_N_BITS),
        .SAMPLE_COUNT_N_BITS    (PK_DET_CNT_N_BITS),
        .PEAK_IDX_N_BITS        (PK_DET_CNT_N_BITS),
        .NUM_WINDOWS_N_BITS     (PK_DET_NUM_WINDOWS_N_BITS),
        .NOISE_EST_N_BITS       (PK_DET_NOISE_EST_N_BITS),
        .BUNDLE_N_BITS          (PK_DET_BUNDLE_N_BITS),
        .DP_ADDR_N_BITS         (4),
        .DP_DIN_N_BITS          (128)
    )
    i_pk_det (
        ...
        .RESET                  (~s00_axi_aresetn),
        .DIN0                   (interp_y[0]),
        .DIN0_DV                (interp_y_dv[0]),
        .DIN1                   (interp_y[1]),
        .DIN1_DV                (interp_y_dv[1]),
        .DIN2                   (interp_y[2]),
        .DIN2_DV                (interp_y_dv[2]),
        .DIN3                   (interp_y[3]),
        .DIN3_DV                (interp_y_dv[3]),
        .DIN4                   (interp_y[4]),
        .DIN4_DV                (interp_y_dv[4]),
        .DIN5                   (interp_y[5]),
        .DIN5_DV                (interp_y_dv[5]),
        .DIN6                   (interp_y[6]),
        .DIN6_DV                (interp_y_dv[6]),
        .DIN7                   (interp_y[7]),
        .DIN7_DV                (interp_y_dv[7]),
        .GUARD_SAMPLES          (pk_det_guard_samples),
        .MODE                   (pk_det_mode),
        .SAMPLE_IDX_OFF         (pk_det_sample_idx_off),
        .SAMPLE_IDX_ON          (pk_det_sample_idx_on),
        .NUM_WINDOWS            (pk_det_num_windows),
        .PROG_FA_THRESH         (pk_det_prog_fa_thresh),
        .PROG_FA_THRESH_EN      (pk_det_prog_fa_thresh_en),

        .BLOCK_VALID            (pk_det_block_valid),
        .DP_ADDR                (),
        .DP_DIN                 (),
        .DP_WE                  ()
    );

...
module bcuda_active_mem #(
    parameter integer C_S_AXI_DATA_DEPTH = 64,
...
(
    ...
    output  wire    [C_S_AXI_DATA_DEPTH - 1 : 0]        READ_ENABLES,
    output  wire    [C_S_AXI_DATA_DEPTH - 1 : 0]        WRITE_ENABLES
);
    ...
    reg     [1 : 0]                         ch0_read_valid;
    reg     [C_S_AXI_DATA_WIDTH - 1 : 0]    ch0_read_data;
    reg     [C_S_AXI_DATA_WIDTH - 1 : 0]    ch0_read_data_pp;
    ...
    begin
        if( S_AXI_ARESETN == 1'b0 )
        begin
            ch0_read_enable    <= 1'b0;
            ch0_read_word_addr <= 'd0;
            ch0_read_valid     <= 2'b00;
            ch0_read_data      <= 'd0;
            ch0_read_data_pp   <= 'd0;
        end
        else
        begin
            ch0_read_enable <= CH0_RD_EN;

            if( CH0_RD_EN )
                ch0_read_word_addr <= CH0_RD_ADDR[C_S_AXI_ADDR_WIDTH - 1 : WORD_ADDR_LSB];

            ch0_read_valid <= {ch0_read_valid[0], ch0_read_enable};

            if( ch0_read_enable )
                ch0_read_data <= regfile[ch0_read_word_addr];

            if( ch0_read_valid[0] )
                ch0_read_data_pp <= ch0_read_data;
        end
    end

    assign READ_ENABLES = (ch0_read_enable << ch0_read_word_addr);
    ...
    assign CH0_RD_VALID = ch0_read_valid[1];

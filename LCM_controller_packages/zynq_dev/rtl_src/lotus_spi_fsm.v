...
(
    ...
    output  reg     [RSP_FIFO_WIDTH - 1 : 0]            RSP_FIFO_WDATA,
    output  reg                                         RSP_FIFO_WEN,
    ...
    localparam integer IDX_ADC0_CHSEL = 1;
    localparam integer IDX_DAISY_EN = 2;

    localparam [$clog2(N_SLAVES) - 1 : 0] SLAVE_IDX_DAISY = 'd3;
    ...
    assign ADC0_CHSEL = SPI_CONTROL[IDX_ADC0_CHSEL];
    ...
    // FIXME: the daisy-chain chip select defaults to low due to the logic of
    // FIXME: the CDCLVC clock buffers. Therefore, we can't dynamically
    // FIXME: control DAISY_CLK_EN. A possible fix is to tie the enable for
    // FIXME: the chip select buffer to VDD so that chip select is always
    // FIXME: propagated.
    assign DAISY_CLK_EN = DAISY_EN;
    // always @( posedge S_AXI_ACLK )
    // begin
    //     if( S_AXI_ARESETN == 1'b0 )
    //     begin
    ...
    //     else
    //     begin
    //         DAISY_CLK_EN <= DAISY_EN &&
    //                         (state != STATE_IDLE) &&
    //                         (SLAVE_IDX == SLAVE_IDX_DAISY);
    //     end
    // end
    ...
    begin
        case(opcode)
            ...
            begin
                ...
                n_frames = 'd0;
                n_wait_cycles = 'd0;
                RSP_FIFO_WEN = !RSP_FIFO_FULL && DONE;
                RSP_FIFO_WDATA = {SLAVE_IDX, RDATA_0};
            end

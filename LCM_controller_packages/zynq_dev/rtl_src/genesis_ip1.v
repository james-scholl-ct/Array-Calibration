...
(
    ...
    //              +----------------+-----------------------+
    // STATUS BLOCK |  (K-1)*L + 0   |  block K-1, word   0  |  RSP_FIFO_POP_WORD
    //              |  (K-1)*L + 1   |  block K-1, word   1  |  RSP_FIFO_PEEK_WORD
    ...
    localparam integer RSP_FIFO_DEPTH = 32;
    localparam integer RSP_FIFO_COUNT_N_BITS = $clog2(RSP_FIFO_DEPTH);
    localparam integer RSP_FIFO_WIDTH = C_S00_AXI_DATA_WIDTH - RSP_FIFO_COUNT_N_BITS - 1;
    ...
    localparam integer RSP_FIFO_POP_WORD    = MEM_STATUS_OFFSET;
    localparam integer RSP_FIFO_PEEK_WORD   = MEM_STATUS_OFFSET + 1;
    ...
    assign active_mem_status_u[0] = {~rsp_fifo_empty, rsp_fifo_count, rsp_fifo_rdata};
    assign active_mem_status_u[1] = {~rsp_fifo_empty, rsp_fifo_count, rsp_fifo_rdata};
    assign active_mem_status_u[2] = {cmd_fifo_full, cmd_fifo_count};
    ...
    // rd_valid indicates that the FIFO output is being read in this cycle
    assign rsp_fifo_ren = active_mem_rd_valids[RSP_FIFO_POP_WORD];
    ...
    // Core
    // ------------------------------------------------------------------------
    assign cmd_fifo_ren = !cmd_fifo_empty && !rsp_fifo_full;
    assign rsp_fifo_wen = cmd_fifo_ren;
    assign rsp_fifo_wdata = cmd_fifo_rdata[RSP_FIFO_WIDTH - 1 : 0];

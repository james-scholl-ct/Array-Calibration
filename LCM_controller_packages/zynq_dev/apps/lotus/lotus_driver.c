...
void axi4lpp_init(struct axi4lpp *pp)
{
    ...
    _axi4lpp_make_map(BSC_ADDR_BASE, &pp->base_bsc, &pp->dev_mem_fd);
    _axi4lpp_make_map(SPI_ADDR_BASE, &pp->base_spi, &pp->dev_mem_fd);
    pp->bsc_clk_freq_MHz = 120.;
    pp->pol_ovr = 0;
}
...
{
    _axi4lpp_close_map(&pp->base_spi);
    _axi4lpp_close_map(&pp->base_bsc);
